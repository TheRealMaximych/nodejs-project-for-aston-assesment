...
Скопируйте `.env.example` в `.env`. Файл `.env` в git не коммитьте. `JWT_SECRET` в примере — заглушка: в локальном `.env` поставьте длинную случайную строку **не короче 32 символов**, реальный секрет не коммитьте. Значение короче 32 символов (например старое `replace-me`) процесс не примет.
...
`migrate` применяет pending-миграции. `seed` создаёт двух демо-пользователей с ненулевым балансом (идемпотентно: повторный запуск не плодит строки). Повторный `seed` **сбрасывает** балансы демо-счетов Alice и Bob на `1000.00` и `500.00`, чтобы снова можно было проверить перевод. Идентификаторы счетов пишутся в лог seed (`accountId`).
